...
if(strcmp(Confirm_Commands_N9310A,"Y"))
    ...
    Error_Code = readline(N9310A_ADDR);
...
N1996A_ADDR.Terminator = "LF";
    ...
    %Sig Gen Ident Check
writeline(N1996A_ADDR, "*IDN?")
fprintf("%s", readline(N1996A_ADDR))
...
if(strcmp(Confirm_Commands_N1996A,"Y"))
    ...
    Error_Code = readline(N1996A_ADDR);
    fprintf("System expirenced error code: %d\nPlease correct This error befor continueing.\n\r", Error_Code);
    pause;
...
Ticks = input("Please indicate how many rotation ticks this test will be preformed across");
Start_Tick = input("Please indicate starting tick (0 mark is 0 tick, anti clockwise is positive, clockwise is negitive)");
...
for ticks = 1:Ticks
    ...
    for sets = Start_Set:End_Set
        ...
        Trace_Data_Query = sprintf(":TRACE:DATA? TRACE%d", Trace_Number);
         writeline(N1996A_ADDR, Trace_Data_Query)
