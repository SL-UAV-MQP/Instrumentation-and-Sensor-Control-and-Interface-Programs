...
% Compensation_Bands = zeros(2, size(88:195:6327,2));
% Compensation_Bands(1,:) = 88:195:6327;
% Compensation_Bands(2,:) = 107:195:6327;
% 
% for aberation = 1:size(Compensation_Bands, 2)
%     Offset = Componant_Sweep_Storage_Clean1a(2,Compensation_Bands(1,aberation)) - Componant_Sweep_Storage_Clean1a(2,Compensation_Bands(1,aberation)-1);
% 
%     Offset_Shift = 1;
%     while(abs(Offset) < 0.2)
%         Offset = Componant_Sweep_Storage_Clean1a(2,Compensation_Bands(1,aberation)) - Componant_Sweep_Storage_Clean1a(2,Compensation_Bands(1,aberation)-1-Offset_Shift);
%         Offset_Shift = Offset_Shift+1;
%     end
%     for compensate = (Compensation_Bands(1,aberation)+1-Offset_Shift):(Compensation_Bands(2,aberation)+1-Offset_Shift)
%         Componant_Sweep_Storage_Clean1a(2,compensate) = Componant_Sweep_Storage_Clean1a(2,compensate) - Offset;
%     end
% end
...
Cable_1_Loss = zeros(Band_End_Last-Band_Start_First+1,2);
Cable_1_Loss(:,1) = Band_Start_First:Band_End_Last;
...
for bsa = 1:size((Band_Start_First:Band_End_Last), 2)
    ...
    x_cord = [Componant_Sweep_Storage_Power_Shift(1,idx(1)), Componant_Sweep_Storage_Power_Shift(1,idx(2))];
    ...
    Cable_1_Loss(bsa, 2) = p_1(1,2)+(((p_t(1,1)-p_1(1,1))*(p_2(1,2)-p_1(1,2)))/(p_2(1,1)-p_1(1,1)));
end
...
sz = 8;
scatter(Componant_Sweep_Storage_Power_Shift(1,:), Componant_Sweep_Storage_Power_Shift(2,:),'.', 'black', 'linewidth', sz)
plot(Cable_1_Loss(:,1), Cable_1_Loss(:, 2), 'blue', 'linewidth', 2)

% for bsa = Band_Start_First:Band_End_Last
%     bsa
%     xline(bsa, 'red');
% end

xlim([750,1400])
ylim([min(Componant_Sweep_Storage_Power_Shift(2,:))-0.5,max(Componant_Sweep_Storage_Power_Shift(2,:))+0.5])
xlabel("Frequency (MHz)")
ylabel("Received Power (dBm)")
...
save(Target_Element, 'Componant_Sweep_Storage_Power_Shift', 'Cable_1_Loss', '-append');